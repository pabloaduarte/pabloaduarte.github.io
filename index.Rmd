...
In China ist eine langsame Erholung bereits zu sehen:
...
Die Google-Trends-Statistik für den Begriff **“Hotel”** zeigt, dass für die Tourismus-Branche keine baldige Erholung in Sicht ist. Nur in **Deutschland** ist ein schwacher Anstieg zu sehen. In Bezug auf das Konsumklima in Deutschland, ist die Intensität der Google-Suchen nach dem Begriff “Restaurant” leicht angestiegen. Die Suche nach „Flug“ und „Bahn“ bleiben in Etwa auf dem April-Niveau.
...
dygraph(hotels_week["20200201/"], main = "Google Trends: 'Hotel'")%>%
dyAxis("x", drawGrid = FALSE)%>%
dyHighlight(highlightSeriesOpts = list(strokeWidth = 3))%>%
dyOptions(colors = c("black","maroon", "gray", "navy", "orange"))%>%
dyLegend(labelsSeparateLines = TRUE, show = "always")%>%
...
dygraph(airbnb_week["20200201/"], main = "Google Trends: 'Airbnb'")%>%
dyAxis("x", drawGrid = FALSE)%>%
dyHighlight(highlightSeriesOpts = list(strokeWidth = 3))%>%
dyOptions(colors = c("black","maroon", "gray", "navy", "orange"))%>%
dyLegend(labelsSeparateLines = TRUE, show = "always")%>%
...
In den USA scheint sich auch das Konsumklima langsam zu erholen.
...
dygraph(us_week["20200101/"], main = "Google Trends: Suchergebnisse in den USA")%>%
  dyAxis("x", drawGrid = FALSE)%>%
  dyHighlight(highlightSeriesOpts = list(strokeWidth = 3))%>%
...
dygraph(germany_week["20200201/"], main = "Google Trends: weitere Ergebnisse in Deutschland")%>%
  dyAxis("x", drawGrid = FALSE)%>%
  dyHighlight(highlightSeriesOpts = list(strokeWidth = 3))%>%
  dyOptions(colors = c("black","maroon", "gray", "navy"))%>%
dyLegend(labelsSeparateLines = TRUE, show = "always")%>%
...
Das Mobilitätsindex für verschienede Städte zeigt, dass Menschen in den Städten weiterhin vorsichtig aggieren.
...
dygraph(request_mobility["202001/"], main = "Citymapper Mobility Index")%>%
dyAxis("x", drawGrid = FALSE)%>% 
dyHighlight(highlightSeriesOpts = list(strokeWidth = 2))%>%
dyOptions(colors = c("black","maroon", "gray", "navy", "orange", "red", "yellow", "green"))%>%
dyLegend(labelsSeparateLines = TRUE, show = "always")%>%
  dyCSS("legend.css")%>%
  dyRangeSelector()
```
