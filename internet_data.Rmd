...
country_names = c("Deutschland", "Frankreich", "Italien", "Spanien", "USA")

job_list<- data.frame(sapply(country, function(x) gtrends(geo = x, cat = 960)))
...
p_joblist_gt <- highchart(type = "stock" ) %>% 
 ...
 hc_subtitle(text = "Quelle: Flossbach von Storch Research Institute, Google Trends.") %>% 
  ...
  hc_rangeSelector(inputDateFormat = '%y-%m-%d', selected = 2) %>%
...
```{r echo=FALSE, results = "hide", message=FALSE, warning=FALSE}
##Bankruptcy
country=c("DE", "FR", "IT", "ES", "US", "GB")
time <- paste("2016-01-01 ",Sys.Date(), sep = "")

insolv<- sapply(country, function(x) gtrends(geo = x, cat = 423, time = time, onlyInterest = TRUE))
...
travel<- data.frame(sapply(country, function(x) gtrends(geo = x, cat = 67, time = time, onlyInterest = TRUE)))
...
shopping<- data.frame(sapply(country, function(x) gtrends(geo = x, cat = 18, time = time, onlyInterest = TRUE)))
...
time <- paste("2016-05-01 ",Sys.Date(), sep = "")
...
destination_week <- as.xts(apply(destination_ts,2, function(x)  x*100/x["2020-01-05"]))
```
...
p_destination_gt <- highchart(type = "stock" ) %>% 
 ...
 hc_subtitle(text = "Quelle: Flossbach von Storch Research Institute, Google Trends.") %>% 
   hc_exporting(enabled = TRUE) %>%
      hc_legend(enabled = TRUE) %>%
  hc_add_series(destination_week$italy, name ="Italien") %>% 
  hc_add_series(destination_week$spain, name = "Spanien" ) %>% 
  hc_add_series(destination_week$greece, name = "Griechenland") %>% 
  hc_add_series(destination_week$france, name = "Frankreich") %>% 
  hc_add_series(destination_week$portugal, name = "Portugal") %>%
  hc_colors(c("black","maroon", "gray", "navy", "orange", "red", "orange"))%>%
  hc_rangeSelector(inputDateFormat = '%y-%m-%d', selected = 2) %>%
  hc_tooltip(valueDecimals= 2)%>%
  hc_chart(style = list(fontFamily = "BigVesta",
...
p_gmob1 <- highchart(type = "stock" ) %>% 
 ...
 hc_subtitle(text = "Quelle: Flossbach von Storch Research Institute, Google COVID-19 Community Mobility Reports.") %>% 
   hc_exporting(enabled = TRUE) %>%
      hc_legend(enabled = TRUE) %>%
...
p_citi <- highchart(type = "stock" ) %>% 
  ...
  hc_subtitle(text = "Quelle: Flossbach von Storch Research Institute, Citymapper Mobility Index") %>% 
  hc_exporting(enabled = TRUE) %>%
      hc_legend(enabled = TRUE) %>%
  hc_add_series(request_mobility$LA , name ="Los Angeles") %>% 
  hc_add_series(request_mobility$NYC, name = "NYC" ) %>% 
  hc_add_series(request_mobility$Paris, name = "Paris") %>% 
  hc_add_series(request_mobility$`Rhine-Ruhr`, name = "Rhine-Ruhr") %>% 
  hc_add_series(request_mobility$HK, name = "Hong Kong") %>%
  hc_add_series(request_mobility$Milan, name = "Milan") %>% 
  hc_add_series(request_mobility$Madrid, name ="Madrid") %>% 
  hc_add_series(request_mobility$London, name ="London") %>% 
  hc_colors(c("black","maroon", "gray", "navy", "blue", "red", "orange"))%>%
  hc_rangeSelector(inputDateFormat = '%y-%m-%d', selected = 2) %>%
  hc_tooltip(valueDecimals= 2)%>%
  hc_chart(style = list(fontFamily = "BigVesta",
                        fontWeight = "bold"))
...
p_hotels <- highchart(type = "stock" ) %>% 
 ...
 hc_subtitle(text = "Quelle: Flossbach von Storch Research Institute, Google Trends.") %>% 
   hc_exporting(enabled = TRUE) %>%
      hc_legend(enabled = TRUE) %>%
  hc_add_series(hotels["201812/"]$DE, name ="Deutschland", connectNulls = TRUE) %>% 
  hc_add_series(hotels["201812/"]$FR, name = "Frankreich", connectNulls = TRUE ) %>% 
  hc_add_series(hotels["201812/"]$IT, name = "Italien", connectNulls = TRUE) %>% 
  hc_add_series(hotels["201812/"]$ES, name = "Spanien", connectNulls = TRUE) %>% 
  hc_add_series(hotels["201812/"]$US, name = "USA", connectNulls = TRUE) %>%
  hc_add_series(hotels["201812/"]$China, name = "China", connectNulls = TRUE) %>% 
  hc_colors(c("black","maroon", "gray", "navy", "orange", "red", "orange"))%>%
  hc_rangeSelector(inputDateFormat = '%y-%m-%d', selected = 2) %>%
  hc_tooltip(valueDecimals= 2)%>%
  hc_chart(style = list(fontFamily = "BigVesta",
...
p_baidu <- highchart(type = "stock" ) %>% 
 ...
 hc_subtitle(text = "Quelle: Flossbach von Storch Research Institute, Baidu-Index") %>% 
   hc_exporting(enabled = TRUE) %>%
      hc_legend(enabled = TRUE) %>%
